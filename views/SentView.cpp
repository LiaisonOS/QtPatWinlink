...
SentView::SentView(PatClient *client, bool touchMode, QWidget *parent)
    : QWidget(parent)
    , m_client(client)
    , m_touchMode(touchMode)
{
    ...
    m_emptyLabel = new QLabel("No sent messages");
    ...
    QObject::connect(m_client, &PatClient::mailboxReady, this, &SentView::onMailboxReady);
    ...
    auto openRow = [this](int row) {
        ...
        *conn = QObject::connect(m_client, &PatClient::messageReady,
            this, [this, conn](const QString &box, const QString &, const QJsonObject &full) {
                ...
                MessageDialog dlg(full, "sent", m_client, m_touchMode, this);
                QObject::connect(&dlg, &MessageDialog::composeRequested, this, &SentView::composeRequested);
                QObject::connect(&dlg, &MessageDialog::deleted, this, &SentView::refresh);
                dlg.exec();
            });
        m_client->fetchMessage("sent", mid);
    };
...
void SentView::refresh()
{
    m_client->fetchMailbox("sent");
}

void SentView::onMailboxReady(const QString &box, const QJsonArray &messages)
{
    if (box != "sent") return;
